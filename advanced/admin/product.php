...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        // Update the product
        $stmt = $pdo->prepare('UPDATE products SET name = ?, description = ?, price = ?, rrp = ?, quantity = ?, img = ?, date_added = ? WHERE id = ?');
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT IGNORE INTO products (name,description,price,rrp,quantity,img,date_added) VALUES (?,?,?,?,?,?,?)');
...
<div class="content-block">
    ...
    <form action="" method="post" class="form responsive-width-100">
        ...
        <textarea name="description" placeholder="Product Description (HTML)"><?=$product['description']?></textarea>
