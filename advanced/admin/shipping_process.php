...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('UPDATE shipping SET name = ?, price_from = ?, price_to = ?, price = ? WHERE id = ?');
        $stmt->execute([ $_POST['name'], $_POST['price_from'], $_POST['price_to'], $_POST['price'], $_GET['id'] ]);
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT INTO shipping (name, price_from, price_to, price) VALUES (?,?,?,?)');
        $stmt->execute([ $_POST['name'], $_POST['price_from'], $_POST['price_to'], $_POST['price'] ]);
        header('Location: index.php?page=shipping');
        exit;
    }
}
...
<?=template_admin_header($page . ' Shipping Method')?>
